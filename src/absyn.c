...
A_exp A_RepeatExp(A_pos pos, A_exp body, A_exp test) {
    ...
    p->kind=A_repeatExp;
    p->pos=pos;
    p->u.repeatt.body=body;
    p->u.repeatt.test=test;
    return p;
}
...
A_exp A_CaseExp(A_pos pos, A_exp exp, A_expList valList) {
    A_exp p = checked_malloc(sizeof(*p));
    p->kind=A_caseExp;
    p->pos=pos;
    p->u.casee.exp = exp;
    p->u.casee.valList = valList;
    return p;
}

A_exp A_CaseValExp(A_pos pos, A_exp val, A_exp action) {
    A_exp p = checked_malloc(sizeof(*p));
    p->kind=A_caseValExp;
    p->pos=pos;
    p->u.caseval.val = val;
    p->u.caseval.action = action;
    return p;
}
...
A_dec A_ConstDec(A_pos pos, S_symbol constt, A_exp init) {
    ...
    p->kind=A_constDec;
    p->pos=pos;
    p->u.constt.constt=constt;
    p->u.constt.init=init;
    p->u.constt.escape=TRUE;
    return p;
}
...
A_ty A_ArrayTy(A_pos pos, A_ty range, A_ty element) {
    ...
    p->kind=A_arrayTy;
    p->pos=pos;
    p->u.arrayy.range = range;
    p->u.arrayy.element = element;
    return p;
}
