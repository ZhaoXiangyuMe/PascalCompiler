...
static char str_oper[][15] = {
	"PLUS", "MINUS", "TIMES", "DIVIDE",  "REALDIVIDE", "MOD",
...
void pr_exp(FILE *out, A_exp v, int d) {
	...
	switch (v->kind) {
		...
		case A_nilExp:
			...
			break;
		case A_realExp:
			fprintf(out, "realExp(%lf)", v->u.real);
			break;
		case A_intExp:
		...
		case A_ifExp:
			...
			indent(out, d+1);
			fprintf(out, "if:\n");
			pr_exp(out, v->u.iff.test, d+2); fprintf(out, ",\n");
			indent(out, d+1);
			fprintf(out, "then:\n");
			pr_exp(out, v->u.iff.then, d+2);
			if (v->u.iff.elsee) { /* else is optional */
				fprintf(out, "\n");
				indent(out, d+1);
				fprintf(out, "else:\n");
				pr_exp(out, v->u.iff.elsee, d+2);
			}
			fprintf(out, ")");
			break;
		case A_whileExp:
		...
		case A_repeatExp:
			...
			indent(out, d+1);
			fprintf(out, "do:\n");
			pr_exp(out, v->u.repeatt.body, d+2);
			fprintf(out, "\n");
			indent(out, d+1);
			fprintf(out, "until:\n");
			pr_exp(out, v->u.repeatt.test, d+2);
			break;
		case A_caseExp:
			fprintf(out, "caseExp(\n");
			pr_exp(out, v->u.casee.exp, d+1);
			fprintf(out, "\n");
			pr_expList(out, v->u.casee.valList, d+1);
			break;
		case A_caseValExp:
			fprintf(out, "caseValExp(\n");
			pr_exp(out, v->u.caseval.val, d+1);
			fprintf(out, "\n");
			pr_exp(out, v->u.caseval.action, d+1);
			break;
		case A_breakExp:
...
static void pr_dec(FILE *out, A_dec v, int d) {
	...
	switch (v->kind) {
		case A_functionDec:
			...
			break;
		case A_constDec:
			fprintf(out, "constDec(%s,\n", S_name(v->u.constt.constt));
			pr_exp(out, v->u.constt.init, d+1); fprintf(out, ",\n");
			indent(out, d+1); fprintf(out, "%s", v->u.constt.escape ? "TRUE)" : "FALSE)");
			break;
		case A_varDec:
